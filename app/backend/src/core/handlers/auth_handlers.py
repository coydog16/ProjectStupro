...
def login_handler():
    ...
    if not user:
        return (
            jsonify({"error": "Authentication failed. Invalid username or password."}),
            401,
        )
...
def register_handler():
    ...
    try:
        ...
        db.session.rollback()
        return (
            jsonify({"error": "User registration failed. Email or username may already exist."}),
            409,
        )
...
def refresh_token_handler():
    ...
    return (
        jsonify({"access_token": access_token, "message": "Access token refreshed"}),
        200,
    )
